...
public class DFS {
     ...
     * dfs 随想
     * <p>首先需要想清楚遍历过程
     * <br>1. 如何构建 & 遍历树？二叉树 —— 左右子树递归；非二叉树 —— 数组形式？其他数据结构形式？怎么表示这个树？for循环子节点进行递归
     * <br>2. 边界条件？一般是达到树叶；二叉树 —— 节点为null；非二叉树 —— 到达边界条件；数组实现一般 height == size()，其他实现
     * <br>3. 如何得到结果？
     * <br>4. 如何进行剪枝？—— 大多时候剪枝是为了得到正确结果，而不是提高性能，所以可以归于 构建树 部分
     *
     * <p>函数传递和保存在函数外一样吗
     * <br>1. 函数传递对象，与保存在函数外，完全一样
     * <br>2. 函数传递不可变类型，此时dfs函数传递，并不会破坏传递对象
     */
